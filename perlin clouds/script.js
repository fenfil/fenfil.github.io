...
let approx = (map, ni, nj) => {
  for (let x = map[ni][nj][0]; x < map[ni][nj + 1][0]; x++) {
    ...
    let r1 =
      map[ni][nj][2] * (1 - (1 - Math.cos(x1 * Math.PI)) * 0.5) +
      map[ni][nj + 1][2] * (1 - Math.cos(x1 * Math.PI)) * 0.5;
    let r2 =
      map[ni + 1][nj][2] * (1 - (1 - Math.cos(x1 * Math.PI)) * 0.5) +
      map[ni + 1][nj + 1][2] * (1 - Math.cos(x1 * Math.PI)) * 0.5;
